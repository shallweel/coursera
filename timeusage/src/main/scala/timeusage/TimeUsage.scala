...
object TimeUsage {
  ...
  /** @return The schema of the DataFrame, assuming that the first given column has type String and all the others
    ...
    */
  def dfSchema(columnNames: List[String]): StructType = StructType(columnNames match {
    case head :: tail =>
      val doubleFields = tail.map(field => StructField(field, DoubleType, nullable = false))
      StructField(head, StringType, nullable = false) :: doubleFields
  })
  ...
  def row(line: List[String]): Row = {
    val values = line match {
      case head :: tail => head :: tail.map(_.toDouble)
    }
    Row(values: _*)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val (primaryNeedsCols, restCols) = columnNames.partition(col => Set("t01", "t03", "t11", "t1801", "t1803").exists(col.startsWith))
    val (workingCols, otherCols) = restCols.partition(col => Set("t05", "t1805").exists(col.startsWith))
    val leisureCols = otherCols.filter(col =>
      Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(col.startsWith))
    def toColumns(names: List[String]): List[Column] = names.map(col)
    (toColumns(primaryNeedsCols), toColumns(workingCols), toColumns(leisureCols))
  }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(row => TimeUsageRow(row.getAs[String]("working"), row.getAs[String]("sex"),
      row.getAs[String]("age"), row.getAs[Double]("primaryNeeds"), row.getAs[Double]("work"), row.getAs[Double]("other")))
