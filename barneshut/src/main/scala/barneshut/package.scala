...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad =
      if (size > minimumSize) {
        val newSize = size / 2
        val fork = Fork(Empty(centerX - newSize / 2, centerY - newSize / 2, newSize),
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = Math.min(Math.max(b.x, boundaries.minX), boundaries.maxX)
      val y = Math.min(Math.max(b.y, boundaries.minY), boundaries.maxY)
      val (ix, iy) = (Math.floor((x - boundaries.minX) / sectorSize).toInt, Math.floor((y - boundaries.minY) / sectorSize).toInt)
      this(ix, iy) += b
      this
    }
