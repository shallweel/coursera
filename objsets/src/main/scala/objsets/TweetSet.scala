...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      val childMost =
        if (left.isEmpty) right.mostRetweeted
      ...
      if (elem.retweets < childMost.retweets) childMost else elem
    }
  }

  def descendingByRetweet = {
    def step(acc: TweetSet): TweetList =
      if (acc.isEmpty) Nil
      else {
        val most = acc.mostRetweeted
        new Cons(most, step(acc.remove(most)))
      }
    step(this)
  }
